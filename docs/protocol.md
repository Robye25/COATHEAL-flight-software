...
| `MOTOR_DEBUG` | `<id>` | Read-only live read of the TMC5160's motion-truth registers for the console's Debug tab: `motor=;sw_pos=;sw_tgt=;sw_hz=;us=;enabled=;moving=;holding=;pulses=;missed=;xactual=;xtarget=;vactual=;mscnt=;tstep=;drv_status=0x…;stst=;cs_actual=;sg_result=;stallguard=;ot=;otpw=;s2ga=;s2gb=;ola=;olb=;s2vsa=;s2vsb=;stealth=;fsactive=;rampstat=0x…;vzero=;pos_reached=;vel_reached=;status_sg=;ioin=0x…;drv_enn=;sd_mode=;version=0x30;gstat=0x…;chopconf=0x…;toff=;mres=;usteps=;resets=`. `sw_*` are the firmware's own counters; `resets` counts chip resets (GSTAT.reset seen after configuration -- VM/VCC_IO dropped) that the firmware recovered from; everything after them comes from the chip (`MSCNT` is the sine-table index that actually drives the coils). NACK `debug registers unavailable` on the simulated backend or a bus error |
