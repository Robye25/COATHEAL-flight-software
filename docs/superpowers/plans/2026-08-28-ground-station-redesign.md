...
- Build/test recipe (Linux dev host):
  ...
  ```
- Baseline at branch: Python 147 tests OK; C++ suite green in CI for `0230cfa`.
...
- [ ] `telemetry_log.py`: `TELEMETRY_CSV_FIELDS` (schema v6 = CLI v5 columns + `stepperN_zeroed`, `stepperN_seq`, `stepperN_seqstate`, `fallback`, `link_loss_s`, `energy_wh`, `budget_wh`, `budget_exhausted`, `heaters_active`, `queue`), `packet_to_row(pkt)`, `TelemetryCsvWriter`, `PullCsvWriter`, `CommandLog` (`ts_utc, command, ok, latency_ms, body, raw`), `EventLog` (`ts_utc level message`), `SessionMeta` (`session.json`).
- [ ] `session_dir.py`: `SessionDirectory.for_session(root, onboard_session_id, now)` → `logs/sessions/<YYYYMMDD-HHMMSS>_<id>/`; reuses an existing directory for the same id; writes `logs/latest_session.txt`.
- [ ] Receiver: open writers lazily on the first frame of a session; switch directory when `session_id` changes; keep `ground_ack_cursor.json` at `logs/`; throttle cursor persistence to ≤1 write/s.
- [ ] CLI server: same writers (delete its inline schema); `--log` now means the sessions root.
- [ ] Dispatcher: every response appended to `CommandLog`; GUI event log appends to `EventLog`.
- [ ] Tests: identical row for identical packet from GUI and CLI paths; directory naming/reuse; command/event files written; cursor throttle. Mutation: drop a v6 column → header assertion names it.
...
- [ ] `StepperStatus` gains `zeroed`, `seq_name`, `seq_state`; segment emits `|zeroed:<0|1>|seq:<name|->|seqst:<idle|run|pause>`.
- [ ] New `CtrlStatus` in `TelemetryRecord` → `,CTRL=fallback:…|link_loss_s:…|energy_wh:…|budget_wh:…|budget_exhausted:…|heaters_active:…|queue:…|plan:none` emitted after `COMPONENT_STATE`, before `STEPPER0`.
- [ ] `parse_telemetry_csv`: `steppers[i]` gains `zeroed: Optional[bool]`, `seq_name`, `seq_state`; `TelemetryPacket.ctrl: Dict[str, str]` (raw) + typed accessors (`fallback_active`, `energy_wh`, …) returning `None` when absent.
- [ ] Tests: old frames (no `CTRL=`, no `zeroed`) still parse with `None`s; new frames populate; C++ serializer test pins the exact token order. Mutation: remove `zeroed` emission → Python test naming the key fails.
...
- [ ] Onboard: `BeaconSenderLoop` and `SendOnboardHelloReply` no-op while `!transmit_enabled_`; the listener still records `latest_gs_`.
- [ ] Onboard: while silent, `HandleCommandLine` accepts only `RADIO_RESUME`, `RADIO_SILENCE`, `STATUS`, `PING`; everything else → `NACK,<cmd>,radio silence active` before any side effect.
- [ ] Onboard: flag file written on `RADIO_SILENCE`, removed on `RADIO_RESUME`; on start, if present → begin silent (decision D2). `STATUS` reply gains `;silence=<0|1>`.
- [ ] GS: `silence_mode` on the dispatcher blocks sends except the whitelist (local `CommandResponse` with error `blocked: radio silence`); beacon/probe threads pause; unknown-at-start handling per spec §9.4.
- [ ] Tests (C++): with transmit disabled, the beacon loop's send counter stays 0 over N iterations and a `GS_HELLO` produces no reply; a `STEPPER_MOVE` during silence returns the NACK and leaves `moving=false`. Tests (Python): dispatcher blocks/allows per whitelist; beacon thread sends nothing while quiet. Mutation: re-enable beacon send → counter test fails.
...
- [ ] `validate_speed_hz(default max 100.0)`; test both sides of the bound.
- [ ] Remove `PHASE_COLORS` Rev-A keys; `phase_color` exact-match on Rev C names.
...
- [ ] Fixed regions per spec §4; all persistent widgets have `objectName`; `saveState/restoreState` round-trips (test: restore after resize changes splitter sizes).
- [ ] Minimum-size test: at 1366×768 and 1920×1080 every region's `minimumSizeHint()` fits its allotted rectangle and no child reports a width larger than its parent (walk the tree). Mutation: give one label a 600 px fixed width → test names it.
- [ ] UI scale actions (`Ctrl+=`, `Ctrl+-`, `Ctrl+0`) change `QApplication.font()` point size within 8–16 and persist.
- [ ] Status bar shows session directory, frame count, parse errors, disk usage — never the stale "start telemetry" text (test).
...
- [ ] Alarm table from spec §5.2; ack semantics (muted until cleared; re-raises when the condition returns after clearing).
- [ ] Top strip fields incl. `T+` from first frame of the session and `RADIO` state; silence band.
- [ ] Beep: `QApplication.beep()` on a *new* alarm only, gated by a persisted toggle (default off).
- [ ] Mutation: remove the `LINK STALE` suppression during silence → test fails.

### Task B3: System tab

**Files:** create `app/gui/tab_system.py`; tests `tests/test_gui_system.py`.

- [ ] Groups per spec §5.3; mode-derived enabling (ARM only in STANDBY, DISARM only in RUN, EXIT SAFE only in SAFE) with reason tooltips; confirm set per §6.1; `CHECK` component selector; last-response line.
- [ ] Tests capture `send`; assert `SET_PHASE <combo>` and confirm-gating; assert disabled states per mode packet. Mutation: allow ARM in RUN → test fails.
...
- [ ] Header (energy bar from `CTRL`, active heaters, PID summary, inhibit); six rows; all-targets; presets with atomic write + `.bak`; `Capture from onboard` parses `GET_THERMAL`.
- [ ] Row state word logic is pure (`heater_state(duty, target, valid, inhibited)`), tested in all five outcomes.
- [ ] Target limits taken from the last `GET_THERMAL` (`target_min_c/target_max_c`) when available.
...
- [ ] Motor cards with `zeroed` from telemetry (grey when the key is absent); selector; actions; jog; speed 1–100 default 100; **BEND** → `STEPPER_MOVETO <id> <target> <hold>`; **STANDARD PULL** → `PULL_EXECUTE <id>`.
- [ ] Gating with reasons (RUN, enabled, zeroed, no fallback, no silence). Mutation: drop the zeroed check → test "BEND enabled on unzeroed motor" fails.
- [ ] Esc / STOP MOTORS keep sending both `STEPPER_STOP` ids (existing tests kept).
...
- [ ] Sequence step speed validated ≤100 Hz before sending.
...
- [ ] Time axis (mission elapsed, UTC in crosshair), window selector, follow/pause, full-session retention, per-tab content per spec §5.6 (Ambient = three x-linked stacked plots), pull markers, targets overlay, export PNG/CSV.
- [ ] Store test: 100k appends stay O(1) amortised (timing bound generous), window slicing exact at boundaries.
...
- [ ] Console entry: completion over `KNOWN_COMMANDS`, history, `Send`, re-issue on row click; every panel's command appears (single dispatcher hook).
- [ ] During silence the console shows the block reason inline.
...
- [ ] Shortcut table per spec §6.5; `P` ignored in text fields (test); no unmodified letter sends a command (test walks all `QShortcut`s).

### Task B11: Docs

- [ ] Rewrite `docs/ground-station.md` and the GUI section of `ground-station/README.md` to the new panels; README command summary; `docs/protocol.md` additions already done in A2/A3.
...
- [ ] D1 Both suites green; mutation notes collected in the PR description.
- [ ] D2 Bench: GUI against the Pi read-only (PING/STATUS/COMPONENTS/CHECK); with the owner: ARM, enable, zero, jog, bend on M1; radio silence with `tcpdump -i eth0 src <pi>` on the Pi showing no onboard-originated packets except replies to the operator's commands.
- [ ] D3 Launchers: `COATHEAL-GroundStation.sh --check` and the Windows `.bat --check` unchanged and passing.
