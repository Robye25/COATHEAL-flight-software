...
| `heater.output_lines` | `19,13,6,5,24,23` | BCM GPIO lines for HEAT_EN1..6 (schematic v3 map). Bench wiring may differ: set the lines the harness really uses here (a heater that warms nothing in `scripts/associate_heaters.py` is usually not on its listed line); `migrate-config` keeps the value from the local config (it is validated, not pinned). The boot-time `gpio=` block is derived from it — redeploy (and reboot) after a change. |
