...
Demo mode feeds the normal GUI with synthetic Rev-C telemetry: pressure
profile, the `PRE_FLOAT` fatigue sequence, sample temperatures held near the
+5 C floor, heater duty cycles, resistance changes, two stepper motors, and
pull events. It also returns local ACKs for command buttons, so the interface
stays responsive without an onboard Raspberry Pi.
