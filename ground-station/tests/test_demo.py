...
class DemoTelemetryTests(unittest.TestCase):
    def test_demo_packet_has_full_ground_station_shape(self) -> None:
        ...
        self.assertIn(pkt.phase, {"ASCENT", "PRE_FLOAT", "FLOAT", "DESCENT", "LANDED"})
        self.assertIn("DEMO_SOURCE", pkt.status)

    def test_demo_enters_rev_c_pre_float_phase(self) -> None:
        scenario = DemoTelemetryScenario(tick_hz=2.0, speed=4.0)
        phases = set()
        for _ in range(24):
            phases.add(scenario.next_packet().phase)
        self.assertIn("PRE_FLOAT", phases)

    def test_demo_emits_pull_events_during_pre_float(self) -> None:
        scenario = DemoTelemetryScenario(tick_hz=2.0, speed=4.0)
        events = []
        for _ in range(60):
